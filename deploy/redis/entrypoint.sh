...
user gateway on >${GATEWAY_REDIS_PASSWORD} %RW~docbase:gateway:* %RW~request_rate_limiter.* %R~docbase:iam:token:auth:* &* +@read +@write +@connection +@transaction +eval +evalsha +info -flushall -flushdb -config

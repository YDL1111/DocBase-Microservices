...
const stubs = { ElEmpty: { props: ["description"], template: "<div><slot />{{ description }}</div>" }, ElSkeleton: { template: "<div />" }, ElTag: { template: "<span><slot /></span>" } };
...
describe("chat message history", () => {
  it("renders numeric role and all message statuses as safe text", () => {
    const wrapper = mount(MessageHistory, { props: { selectedSessionId: 1, loading: false, messages: [message(1, 1), message(2, 2), message(3, 3), message(2, 4)] }, global: { stubs } });
